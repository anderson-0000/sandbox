...
const PlanetSelector: React.FC = () => {
  const { selectedObjectName, setSelectedObjectName } = useStore();
  ...
  return (
    <div className="absolute top-8 right-8 flex flex-col items-end gap-2 z-[60] pointer-events-auto h-[80vh] overflow-y-auto pr-4 scrollbar-hide">
      ...
      <div className="flex flex-col gap-4 items-end">
        {/* Sun */}
        <button
          onClick={() => setSelectedObjectName(selectedObjectName === 'Sun' ? null : 'Sun')}
        ...
        {Object.values(PLANETS_DATA).map((planet) => {
          ...
          const isSelected = selectedObjectName === planet.id;

          return (
            <div key={planet.id} className="flex flex-col items-end gap-2">
              <button
                onClick={() => setSelectedObjectName(isSelected ? null : planet.id)}
                className={`px-4 py-2 rounded-full text-xs font-bold transition-all border ${
                  isSelected
              ...
              {moons.length > 0 && (
                <div className="flex flex-wrap gap-1.5 justify-end max-w-[200px] mr-2">
                  {moons.map((moon) => {
                    const isMoonSelected = selectedObjectName === moon.id;
                    return (
                      <button
                        key={moon.id}
                        onClick={() => setSelectedObjectName(isMoonSelected ? null : moon.id)}
